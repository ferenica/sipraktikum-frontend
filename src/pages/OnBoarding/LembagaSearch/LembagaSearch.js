...
import AddLembagaCard from './AddLembagaCard/AddLembagaCard';
 ...
 * @parent OnBoarding.js
 * @param {*} event
 * @param {*} key
 */
class LembagaSearch extends Component {
  newDate = new Date();  
  ...
  state = {
    ...
    defaultTahun: ['', this.year, this.year - 1, this.year - 2, this.year - 3, this.year - 4, this.year - 5],
    selectedTahun: ['', this.year, this.year - 1, this.year - 2, this.year - 3, this.year - 4, this.year - 5]
  }
    ...
    axios.all([
      ...
      axios.get(this.URL_TEMA),
        ...
        .then(axios.spread((institusi, tema) => {
          const filterData = {};
          filterData['tahun'] = [
            {id: 1, nama: (this.year - 5).toString(), isChecked: false},
            {id: 2, nama: (this.year - 4).toString(), isChecked: false},
            {id: 3, nama: (this.year - 3).toString(), isChecked: false},
            {id: 4, nama: (this.year - 2).toString(), isChecked: false},
            {id: 5, nama: (this.year - 1).toString(), isChecked: false},
            {id: 6, nama: this.year.toString(), isChecked: false},
          ];
  ...
  redirectArsipLembaga = () => {
    window.location = '/arsip-lembaga';
  }

  /**
   * @return {*} lembaga and the filter components
   */
  render() {
    ...
    if (!this.state.isLoaded) {
      ...
      insidePaper = (
        <Grid container item cols={{lg: 12, md: 10, sm: 6, xs: 3, xxs: 2}}>
          {
            this.props.isAdmin ? (
                <AddLembagaCard className="add-lembaga" />
            ) : null
          }
          {
    ...
    return (
      <Grid className="root" container style={{position: 'relative'}}>
        <Grid item xs={2} sm={3}>
          <StyledPaperFilter>
            {
          ...
          <StyledContainer>
            <Button
              style={{marginBottom: '1.5rem', borderRadius:'20px'}}
              variant='contained'
              color='primary'
              onClick={this.redirectArsipLembaga}>
              Arsip Lembaga
            </Button>
          </StyledContainer>
        </Grid>
...
export default LembagaSearch;

LembagaSearch.propType = {
